...
import { State, Party } from './enum/index.js';
import Role from './role/index.js';
import { Normal } from './world/index.js';
import { gameConfig, dataSetup, symbols, guide, vietsub } from './helper.js';
import StateManager from './State.js';

// const { this.sendMessage } = global.api;
...
export default class MasoiGame extends Game {
	constructor(options = {}) {
		super({
			...options,
			...{
		...
		this.userName = {};
		this.userName[this.masterID] = global.data.users.get(this.masterID).info.name
		if (options.param[0] == 'info') {
			let indexVillage = Number(options.param[1]) - 1;
			if (!options.param[1]) return this.sendMessage(`Lệnh: ${prefix}masoi info [Mã làng]`, this.threadID);
			if (!gameConfig.setups[indexVillage]) return this.sendMessage(`Không tìm thấy làng với mã số ${symbols[options.param[0]]}!`, this.threadID);
			...
			return this.sendMessage(msg, this.threadID)
		}
		...
		if (!options.param[0] || isNaN(indexVillage)) {
			...
			return this.sendMessage(body.replace(/,/g, ""), this.threadID);
		}

		if (!gameConfig.setups[indexVillage]) {
			return this.sendMessage(`Không tìm thấy làng với mã số ${symbols[options.param[0]]}!`, this.threadID);
		}
		this.setup = dataSetup(gameConfig.setups[indexVillage]);
		this.state = new StateManager([State.SETUP, State.PLAY]);
		this.world = new Normal({
			game: this
		});
	...
	async stateSetup(message) {
		...
		if (message.body.toLowerCase() == gameConfig.ready && this.participants.length < this.setup.roles.length && !this.participants.includes(message.senderID)) {
			...
			this.userName[message.senderID] = global.data.users.get(message.senderID).info.name
				...
				for (let i = 0; i < this.participants.length; i++) {
					...
					const player = this.world.add(
						new Role[this.setup.roles[i]]({
							...
							world: this.world,
							name: this.userName[participantID] || '<Chưa kết bạn>',
	...
	async sendMessage(message, threadID = this.threadID) {
		await this.send(message, threadID);
	}
	...
	listPlayer(filter = {}) {
		...
		for (let index = 0; index < this.world.getLength(); index++) {
			...
			if (pass)
				text += `${symbols[index + 1]} ${player.name} ${player.died ? ' - đã chết' : ''
					}\n`;
		}
		return text;
	}
