import { DeathType, Party } from '../enum/index.js';
import { validIndex, alive, diff } from '../format/index.js';
...
export default class Pair extends Ability {
	...
	static check(player, value) {
		const pairs = value
			.split(' ')
			.slice(0, 2)
			.map(val => player.format(val, validIndex, alive));
		if (pairs.length != 2) {
	...
	static async nightend(player, pairs) {
