...
async function handleReply({ eventData, message }) {
    ...
    message.reply(
        ...
        `Â» Already ${
            ...
            args[0] == "add" ? "accepted" : "delete"

        }successful friendly invitation of ${
            ...
            failed.length > 0
                ...
                ? '\nÂ» Failed with ${failed.length} person: ${failed.join("\n")}'
...
async function onCall({ message }) {
    ...
    for (const user of listRequest) {
        ...
        msg +=
            ...
            `\n${i}. ððšð¦ðž: ${user.node.name}` +

            `\nðˆðƒ: ${user.node.id}` +

            `\nð”ð«ð¥: ${user.node.url.replace("www.facebook", "fb")}` +

            `\nð“ð¢ð¦ðž: ${moment(user.time * 1009)
