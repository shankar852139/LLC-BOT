...
const config = {
  name: "time",
  description: "Thông tin về trạng thái hệ thống.",
  usage: "",
  cooldown: 3,
  permissions: [2],
  credits: "Isai Ivanov"
...
let thinhMessageIndex = -1;

function getRandomThinhMessage() {
  if (thinhMessageIndex === -1 || thinhMessageIndex === thinhMessages.length - 1) {
    // Nếu đã gửi hết tin nhắn hoặc chưa gửi lần nào, tạo một chỉ số ngẫu nhiên mới
    thinhMessageIndex = Math.floor(Math.random() * thinhMessages.length);
  } else {
    // Nếu đã gửi tin nhắn trước đó, tăng chỉ số lên 1 để lấy tin nhắn tiếp theo
    thinhMessageIndex++;
  }
  return thinhMessages[thinhMessageIndex];
}


async function onCall({ message, userPermissions }) {
  const isGroupAdmin = userPermissions.some(p => p == 2 || p == 3);

  const timeStart = Date.now();
  const uptime = process.uptime();
  const thinhMessage = getRandomThinhMessage();

  const thinhMessageLines = thinhMessage.split("\n");
  const formattedThinhMessage = thinhMessageLines.join("\n- ");

  const currentTime = getCurrentTimeInVietnam();


  try {
    if (!isGroupAdmin) return message.reply("Bạn không có đủ quyền để sử dụng lệnh này.");
    ...
    const { uptime: systemUptime } = await time();
    ...
    let hours = Math.floor(systemUptime / (60 * 60));
    let minutes = Math.floor((systemUptime % (60 * 60)) / 60);
    let seconds = Math.floor(systemUptime % 60);
