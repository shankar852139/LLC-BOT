...
async function installDependencies(dependencies, pluginName, plgPath) {
    ...
    if (notInstalled.length > 0) logger.custom(`Tải phần phụ thuộc cho  ${fileName}`, 'LOADER', '\x1b[33m');
    else return true;

    for (const dependency of notInstalled) {
        try {
            execSync(`npm install --package-lock false ${dependency}`, { stdio: 'ignore' });
            logger.custom(`Tải thành công phần phụ thuộc ${dependency}`, 'LOADER', '\x1b[32m');
            installed.push(dependency);
        } catch (err) {
            logger.custom(`Không thể tải phần phụ thuộc ${dependency}`, 'LOADER', '\x1b[31m');
...
async function loadPlugins() {
    logger.custom('Đọc phần phụ thuộc của plugin...', 'LOADER', '\x1b[33m');
    const pluginsPath = resolvePath(process.cwd(), 'plugins');
    const plugins = ["commands", "events", "customs", "onMessage"];

    for (const plugin of plugins) {
        logger.custom(`Tiến hành đọc phần phụ thuộc cho  ${plugin}...`, 'LOADER', '\x1b[33m');
